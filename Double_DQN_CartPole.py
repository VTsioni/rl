# Double DQN implementation with OOP
# use of the Huber loss and the minimisation of the
# # temporal difference error, δ, and that means we implement the prioritised experience replay
...
def optimize(batch_size=Batch_size, gamma=Gamma):
    ...
    if memory.can_provide_sample(batch_size):
        ...
        next_best_q_states = policy_net(non_final_next_states)
        next_best_actions = torch.argmax(next_best_q_states, axis=1)

        next_best_q_values = torch.zeros(batch_size, device=device)
        with torch.no_grad():
            next_best_q_values[non_final_mask] = target_net(non_final_next_states)[np.arange(0, next_best_actions.size(dim=0)), next_best_actions]


        # calculate the target Q-values (aka. Bellman Optimality Equation)
        target_q_values = reward_batch + (gamma * next_best_q_values)
